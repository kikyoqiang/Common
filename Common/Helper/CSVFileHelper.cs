    ...
    public class CSVFileHelper
    {
        ...
        public static DataTable OpenCSV(string filePath)
        {
            ...
            while ((strLine = sr.ReadLine()) != null)
            {
                ...
                if (IsFirst == true)
                {
                    tableHeads = strLine.Split(',').Select(a => a.Trim('"')).ToArray();
                    IsFirst = false;
                    columnCount = tableHeads.Length;
                    //创建列
                    for (int i = 0; i < columnCount; i++)
                    {
                        DataColumn dc = new DataColumn(tableHeads[i]);
                        dt.Columns.Add(dc);
                    }
                }
                else
                {
                    aryLines = strLine.Split(',');
                    if (aryLines.Length < columnCount)
                        continue;

                    DataRow dr = dt.NewRow();
                    for (int j = 0; j < columnCount; j++)
                    {
                        dr[j] = aryLines[j].Trim('"');
                    }
                    dt.Rows.Add(dr);
                }
            }
            ...
            {
                dt.DefaultView.Sort = tableHeads[0] + " " + "asc";
            }
